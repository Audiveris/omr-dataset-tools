...
/**
 * Class {@code DoubleFunction} represents a function y = f(x), where x is integer
 * and y are doubles.
 * <p>
...
public class DoubleFunction
{
    ...
    private static final Logger logger = LoggerFactory.getLogger(DoubleFunction.class);
    ...
    /**
     * Creates an instance of {@code DoubleFunction}.
     *
     * @param xMin minimum x value
     * @param xMax maximum x value
     */
    public DoubleFunction (int xMin,
                           int xMax)
    {
        this.xMin = xMin;
        this.xMax = xMax;
        values = new double[xMax - xMin + 1];
    }
        ...
        Collections.sort(maxima, new Comparator<Integer>()
                 {
                     ...
                     public int compare (Integer x1,
                                         Integer x2)
                     {
                         return Double.compare(getValue(x2), getValue(x1)); // Reverse
                     }
                 });
    ...
    public void print (PrintStream stream)
    {
        ...
        for (int x = xMin + 1; x <= xMax; x++) {
            double der = getDerivative(x);
            stream.format(" %d:%f/%+f%n", x, getValue(x), der);
        }
