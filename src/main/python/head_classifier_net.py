# ==================================================================================================
#
#                            h e a d _ c l a s s i f i e r _ n e t . p y
#
# ==================================================================================================
# Prototype model for Audiveris head classifier based on MobileNet architecture
#
# This code is closely derived from a stripped-down version of existing mobilenet_v2.py
# also inspired from minimalistic instance of MobileNetV3Small found in mobilenet_v3.py
#
# ==================================================================================================
# License for existing mobilenet_v2:
#
...
def HeadClassifierNet(input_shape=None,
                alpha=1.0,
                include_top=True,
                weights='imagenet',
                input_tensor=None,
  ...
  x = _inverted_res_block(x, filters=192, alpha=alpha, stride=1, expansion=6, block_id=10)
  x = _inverted_res_block(x, filters=192, alpha=alpha, stride=1, expansion=6, block_id=11)
  x = _inverted_res_block(x, filters=192, alpha=alpha, stride=1, expansion=6, block_id=12)
...
def _inverted_res_block(inputs, expansion, stride, alpha, filters, block_id):
  ...
  print('block_id:', block_id, 'in_channels:', in_channels) # HB
...
if __name__ == "__main__":
    # Meant for general patch classifier
    #model = HeadClassifierNet(input_shape=(184, 56, 1), classes=???, weights = None)
    
    # Meant for head classifier
    _rows = 54
    _cols = 54
    _shapes = 12
    _name='headclassifier_h{}_w{}_s{}'.format(_rows, _cols, _shapes)
    model = HeadClassifierNet(input_shape=(_rows, _cols, 1), classes=_shapes,
        weights = None, classifier_activation='softmax')
    model.compile(loss = 'categorical_crossentropy', optimizer = 'sgd', metrics = ['accuracy'])
    model.summary()
#    tf.keras.utils.plot_model(model, to_file='build/' + _name + '.png', 
#        show_shapes=True, show_dtype=False,show_layer_names=True, rankdir='TB', expand_nested=False,
#        dpi=192)
#    model.save('build/' + _name + '.h5')
